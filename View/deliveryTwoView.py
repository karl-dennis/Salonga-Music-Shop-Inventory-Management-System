...
class deliveryTwoView(ctk.CTkFrame):
    ...
    def show_secondPage(self):
        self.secondPageFrame = ctk.CTkFrame(self.baseFrame, width=522, height=583, fg_color='#F7F7F7', bg_color='#DFDFDF', corner_radius=7)
    ...
    def show_historyTable(self):
        ...
        for row_index, row_values in enumerate(self.reordered_table):
            ...
            self.table.insert(row_index, 2,
                              f"${row_values['totalRevenue']:.2f}".rstrip('0').rstrip('.'))  # Format revenue
            self.table.insert(row_index, 3, row_values['status'])  # Insert status_text directly
        ...
        for row in range(self.table.rows):
            ...
            status_text = self.reordered_table[row]['status']
